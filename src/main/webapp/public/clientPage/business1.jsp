...
<script type="text/javascript">
	...
	function add_dishNum(){
		...
		var obj = document.getElementsByClassName('mui-table-view-cell');
...
-->
		...
		<div class="container">
			<div class="row titll">
				<div class="col-xs-4" style="line-height: 3em;">
					<div
						style="width: 60%; font-size: 16px; margin-left: 20%; border-bottom: 3px #FFA980 solid; color: #FFA980"
						class="shangjia_caidan_top shangjia_caidan_top_left">点菜</div>
				</div>
				<div class="col-xs-4" style="line-height: 3em">
					<div style="width: 60%; margin-left: 20%; font-size: 16px;"
						class="shangjia_caidan_top shangjia_caidan_top_right"
						onClick="showStoreInfo();">商家</div>
				</div>
			</div>
		</div>
	</div>
	...
	<div>
		<div class="lie">
			<div class="mui-content mui-row mui-fullscreen search-menu">
				...
				<div id="segmentedControlContents" class="mui-col-xs-9"></div>
			</div>
		</div>

		<div class="shop">
			<img src="images/gdsgf4.jpg" class="img_sh">
			<div class="sh_name"></div>
		</div>
	</div>
	...
	<div class="gou">
		<p class="wu_t">
			<i></i>购物车
			<!--<a href="#"><span class="iconfont icon-iconfontshanchu"></span>清空</a>-->
		</p>
		<div class="container">
	...
	<script type="text/javascript">
		//显示餐厅的所有菜品
		function showDish(typeId){
			var caidanlist = new Array();  
		    $.ajax({  
		        url: "../../dishInfo/dishlist",  
		        async:false, 
		        dataType:'json',
		        data: {store_id:<%=store_id%>,dishtypeId:typeId},  
	...
	<script>
			...
			//显示右侧菜品类型
			console.log(showDishType());
			var dishtypeArray = new Array();
			var typeIdArray = new  Array();
			//创建新的数组添加-->dishtype
			for(var num=0;num<showDishType().length;num++){
				dishtypeArray.push(showDishType()[num].dishtype);
				typeIdArray.push(showDishType()[num].dishtypeId);
			}
			var menulist = [].concat(dishtypeArray);
			var html = [];
			var i = 1;
			var j = 1;
			var	m = menulist.length+1; //左侧选项卡数量+1
			var	n = menulist.length+1; //每个选项卡列表数量+1
			for (; i < m; i++) {
			...
			for (i = 1; i < m; i++) {
				...
				for (j = 1; j < n; j++) {
					var small= new Array("莓类","百香果","草莓","橙.柚.柑","瓜类","梨","芒果","苹果","西红柿");
				    var imgArr = ['shangcheng/images/f1.jpg','shangcheng/images/f2.jpg','shangcheng/images/f3.jpg','shangcheng/images/f4.jpg','shangcheng/images/f5.jpg','shangcheng/images/f6.jpg','shangcheng/images/f7.jpg','shangcheng/images/f8.jpg','shangcheng/images/f9.jpg'];
					var typeId = typeIdArray[i-1];
				    html.push('<li class="mui-table-view-cell">' +showDish(typeId).join('') + '</li>');
				}
				html.push('</ul></div>');
			}
