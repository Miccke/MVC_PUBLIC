...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    ...
    renderDOM: function () { // 渲染DOM
	  ...
	  var queren = '<p style=" font-size:20px; color:#ff808e; text-align:center; width:100%" onClick="submitTime();">确定</p>'

      this.$calendar_title.html(_titleStr);
      this.$calendar_week.html(_weekStr);
      this.$calendar_date.html(_dateStr+queren);
...
//      });
      ...
      this.$calendarDate_item.click(function () {
		...
		time = addMark($(this).attr('data'))
		
		console.log(addMark($(this).attr('data'))); // 今天的日期
...
function submitTime(){
	if(checkTime(time)){
		document.getElementsByClassName('timeshow')[0].innerText = time;
...
function checkTime(times){
	 ...
	 var timeStamp = Date.parse(new Date(times));
	 if(date>timeStamp){
		 alert('选择时间不能小于当前时间');
		 return false;
	 }
