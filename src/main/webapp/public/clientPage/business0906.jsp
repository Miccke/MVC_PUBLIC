...
<script type="text/javascript">
	...
	function showStoreInfo(){
		$.get("../../storeInfo/singleStore", {
			...
			var storeInfo = '<div class="container" style="width: 100%; float: left; margin-top:2.5em">'
					...
					+'<div class="zero" style="width: 100%; border-top: 1px solid #f3f3f3; float: left">'
					+'<span class="fu" style="margin-top: 8px; margin-left: 15px;">'
					+'<img src="images/bussins/shop_server.png" width="15" height="15" />'
					+'<span style="font-size: 14px; margin-left: 20px">'
...
<body>
		    ...
		    $.ajax({  
		        ...
		        success: function (data) {  
		        	for (var i = 0; i < data.length; i++) {
		        		//alert(data[0])
						caidanlist.push(
							'<li code="'+data[i].dishName+'" class="mui-table-view-cell caiming_name"  price="'+data[i].dishPrice+'" id="'+data[i].dishId+'">'
	...
	<script>
			...
			html = [];
			for (i = 1; i < m; i++) {
				html.push('<div id="content' + i + '" class=" "><ul class="mui-table-view">'+'<h2><span class="fl">' + menulist[i-1] + '</span></h2>');//分类
				//for (j = 1; j < n; j++) {
					var typeId = typeIdArray[i-1];
				    html.push(showDish(typeId).join(''));
				//}
				html.push('</ul></div>');
			}
	...
	<script type="text/javascript">
		function dingdanSubmit() {
			...
			if(s3.rows.length>0){
				...
				if(parseInt(miniMoney)>=parseInt(dataObject.amount)){
					alert('未达到商家起送价，再拼一下');
				}else{
					$.ajax({
